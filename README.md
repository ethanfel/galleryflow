...
- Find hard-to-name poses from your own examples with pose-first ranking: RTMO-L supplies confidence-aware multi-person geometry, spatial DINOv2 handles uncertain or badly cropped detections as a lower-priority fallback, and perceptual hashes recognize true source-image matches.
...
- Refine each pose from reviewed results: curate the exact gallery images that are genuinely correct, keep uncertain galleries neutral, reject false matches, and apply bounded pose-only feedback to future scans without retraining the vision models.
...
The three proposed matches on each result are independently checkable and start checked. Uncheck an incorrect image before reviewing the gallery: only checked images are saved as pose feedback. After accepting or rejecting, edit the checks and use **Save selection**. If the scanner chose an adjacent frame, open the gallery in Finder review and choose up to three images from anywhere in that gallery. GalleryFlow attempts pose analysis for newly chosen images, but your manual selection is always saved even when the model or media is temporarily unavailable or RTMO cannot confirm a reliable pose. Such images are labeled **Saved · pose pending**, remain available for control/target preparation, and are currently excluded from automatic Finder reranking; a later successful pose analysis can make them ranking-eligible. **Accept** requires at least one selected image.
...
Feedback is scoped to the pose tag and uses stored RTMO geometry only; it does not train RTMO or DINOv2, and appearance similarity never becomes a feedback signal. Two distinct galleries with usable pose detections activate either the accepted or rejected side independently. The adjustment is capped at ±0.08 and only reorders reliable results inside their existing pose lane: exact matches, visual fallback, and the pose-match tier boundaries remain authoritative. Every new scan snapshots the current feedback revision, so later reviews or a reset cannot change that scan midway through a pause, resume, or extension.

The Finder setup panel shows accepted/rejected sample counts, usable-gallery progress, and a pose-specific reset action. Resetting removes learned feedback without deleting galleries, cached descriptors, or historical review states. On upgrade, existing accepted and rejected results are migrated from their retained top matches; usable pose data already present in the current cache is preserved automatically, while missing or unreliable pose snapshots simply do not count toward activation.
