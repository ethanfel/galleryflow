...
Open a gallery and switch it to **Pose dataset** mode. Targets and controls can be assigned in whichever order is convenient: check one or more target images, choose their primary pose tag and intended solo, couple, or group role, then press **Apply targets**; the matching control can be added afterward. To assign a control without opening the full-size viewer, check one thumbnail and press **Set solo control**, **Set couple control**, or **Set group control**. The Pose field is a searchable reusable library ordered by recent use. Existing first-level folders below the configured `pose_pairs` root are imported automatically, so an older or manually created dataset folder is immediately available in every gallery. A trailing slash typed after a pose name is ignored. Each gallery draft remains scoped to its selected profile. The revisioned draft is saved on the server as you work, including temporarily incomplete target-first drafts, so closing the modal or browser does not discard it.

Each target has exactly one pose tag and uses exactly one of the gallery's three control alternatives. Before export, GalleryFlow reports incomplete targets whose selected role has no control. **Download & organize** snapshots the current draft and sends the work through the persistent job queue, where progress, cancellation, and restart recovery behave like gallery downloads. GalleryFlow indexes existing pose target filenames and records every successful future export. Finder cards identify galleries that already produced pose images; the gallery summary and image thumbnails show the exact poses and ordinals already exported. Re-exporting a mixed draft skips exact existing pairs while still queuing new pairs, and moving an already exported image to a different pose is blocked instead of silently duplicating or overwriting it. In a Finder review, queuing the dataset leaves the gallery open, marks that exact saved draft **Queued**, and keeps Previous/Next available so review can continue without returning to the result grid.
...
Results remain ordinary galleries. Pending and **Maybe** results open with the scanner suggestions highlighted in Pose dataset mode. Accepted and rejected results open in **Finder review**, where the saved feedback subset can be edited against the complete gallery. Every Finder-opened gallery now keeps a review rail above the workspace: **Accept**, **Maybe**, and **Reject** save the decision without closing the gallery, while Previous/Next (or the left/right arrow keys) move through the current filtered review queue. The queue remains stable while decisions remove cards from the underlying tab, pose drafts finish saving before navigation, and a successful pose export stays in the same window with its button locked as **Queued**. GalleryFlow prefetches the adjacent gallery details and one low-priority preview per neighbor, so moving backward or forward normally reuses warmed data without letting speculative images monopolize the browser's HTTP connections. All managed previews share a three-request scheduler, visible gallery images take priority, stale requests are aborted on navigation, and state-only updates retain already decoded image nodes instead of flashing and requesting every thumbnail again. The result grid waits while the gallery modal is open. The cache is profile-aware, bounded, and disabled for preview images when the browser reports Data Saver, a 2G connection, or a hidden tab. Gallery persistence and health database reads also run outside the asynchronous request loop, keeping review controls responsive when storage is briefly busy. This makes it possible to review, assign targets before or after controls, export, then continue without returning to the result grid after every gallery.
...
Scans, corpus mappings, results, and pose feedback live in SQLite under `/data` and survive browser or container restarts. Keep the same `/data` volume or bind mount when replacing the container; this feature requires no additional Docker mount. Current `pose-precision-v2` scans can be paused, resumed, canceled, and filtered by review state or match score. A failed current scan exposes **Retry search**, which requeues the same scan from its saved source cursor while preserving its query, candidates, reviews, page progress, and completed local-index work; an already completed local phase is not scanned again. Finder results are server-paged 24 galleries at a time: changing the review tab, score threshold, or page queries only that slice from SQLite instead of loading every candidate into the browser. Results from earlier appearance-first and `pose-first-v1` scans remain viewable after upgrading, but cannot be resumed or extended; start a new scan to avoid mixing ranking systems. Category, tag, and pasted `/?q=...` search scans follow PornPics' JSON infinite-scroll cursor beyond the first 20 HTML cards; search terms are preserved when the HTML page switches to the JSON endpoint, and repeated gallery IDs are skipped automatically. When a completed or paused current scan still has another page in its current source, **Extend search** adds up to 50 pages to that same scan and resumes its saved cursor without revisiting completed galleries or losing review decisions. A paused, canceled, failed, or completed current scan also exposes **Switch source**, so an unwanted or broken source can be replaced immediately without spending its remaining page budget. The new PornPics home, search, category, gallery, or other supported browse URL reuses the same scan ID, query, cached local-index work, candidates, reviews, reference snapshot, and feedback snapshot. When a completed scan has exhausted its source, the same control is labeled **Explore another source** and **Continue same scan**. Source switches survive restarts, deduplicate galleries already scanned, and keep the cumulative scan capped at 500 completed pages.
