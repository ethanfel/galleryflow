...
- Refine each pose from reviewed results: keep the suggested images that are genuinely correct, reject false matches, and apply bounded pose-only feedback to future scans without retraining the vision models.
...
Results remain ordinary galleries. **Open gallery** loads the complete gallery in Pose dataset mode, checks and highlights the matching image, and fills in the pose tag. You can then find the related solo, couple, or group control image in the same gallery and explicitly apply or export the pair. Accepting or rejecting a Finder suggestion never downloads, globally ignores, or silently tags a gallery.

The three proposed matches on each result are independently checkable and start checked. Uncheck an incorrect image before reviewing the gallery: only the images still checked are saved as pose feedback. **Accept** requires at least one checked suggestion. **Reject** may be saved with none checked when you only want to organize the review queue without teaching a negative example.
